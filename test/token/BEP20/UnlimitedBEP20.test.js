...
const UnlimitedBEP20 = artifacts.require('UnlimitedBEP20');
const ServiceReceiver = artifacts.require('ServiceReceiver');

contract('UnlimitedBEP20', function ([owner, other, thirdParty]) {
  const _name = 'UnlimitedBEP20';
  const _symbol = 'BEP20';
  ...
  beforeEach(async function () {
    ...
    await this.serviceReceiver.setPrice('UnlimitedBEP20', fee);
  });

  context('creating valid token', function () {
    describe('as a UnlimitedBEP20', function () {
      describe('without initial supply', function () {
        beforeEach(async function () {
          this.token = await UnlimitedBEP20.new(
            _name,
            _symbol,
            _decimals,
            0,
  ...
  context('UnlimitedBEP20 token behaviours', function () {
    beforeEach(async function () {
      this.token = await UnlimitedBEP20.new(
        _name,
        _symbol,
    ...
    context('like a UnlimitedBEP20', function () {
